#!/usr/bin/awk -f
BEGIN{	gen_output="po_files";print > gen_output;"mkdir llixhoot"|getline}
{
	if ( $0 ~ /i18n={/ ) a=1;
	if ( a >=1 ) 
	{ 
		if ($0 ~ /.*\".*:.*\".*/ )
		{
			...
			if ( $0 ~ "\"..\":" )
			{
				...
				if ( $0 ~ /\"[a-zA-Z0-9_]*\"/ && a>=1)
				{
					...
					if ( a==1 )
					{
						"mkdir llixhoot/"fname[2]|getline;
						output="llixhoot/"fname[2]"/"fname[2]".pot";
						lang="";
						fuzz="#, fuzzy"
					} else {
						lang="es_ES";
						fuzz=""
						output="llixhoot/"fname[2]"/es_ES.po";
					}
